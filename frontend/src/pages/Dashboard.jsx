...
const StatCard = ({ label, value, icon: Icon, onClick }) => (
  <div
    ...
    className={`group bg-white border rounded-md p-5 transition-all ${
      onClick
      ...
      <div className="flex items-center gap-1">
        ...
        {onClick && (
          <ChevronRight
            size={14}
            strokeWidth={2}
            className="text-indigo-500 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all"
          />
        )}
      </div>
    </div>
...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-end justify-between flex-wrap gap-4 pb-6 border-b border-slate-200">
        <div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {sortedTeam.map((row) => (
                  ...
                  >
                    <td className="px-4 py-2.5 font-medium text-slate-800">{row.user.name}</td>
                    <td className="px-4 py-2.5 text-slate-500">{row.user.role}</td>
                    ...
                    <td className="px-4 py-2.5 text-right tabular-nums">{fmtCurrency(row.wonRevenue)}</td>
                  </tr>
                ))}
                {sortedTeam.length === 0 && (
                  <tr>
